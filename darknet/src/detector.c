...
void validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh)
{
    ...
    const float f1score = 2.F * cur_precision * cur_recall / (cur_precision + cur_recall);
    printf(" for thresh = %1.2f, precision = %1.4f, recall = %1.4f, F1-score = %1.4f \n",
        thresh_calc_avg_iou, cur_precision, cur_recall, f1score);
...
void run_detector(int argc, char **argv)
{
    ...
    float thresh = find_float_arg(argc, argv, "-thresh", .5);    // 0.24
