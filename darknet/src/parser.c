...
network parse_network_cfg_custom(char *filename, int batch)
{
    ...
    float bflops = 0;
    unsigned long int num_parameters = 0;
    size_t workspace_size = 0;
    n = n->next;
    int count = 0;
    free_section(s);
    fprintf(stderr, "layer     filters    size           input                 output          BFLOPs      parameters\n");
    while(n){
        params.index = count;
        ...
        if (l.num_parameters > 0) num_parameters += l.num_parameters;
    }
    free_list(sections);
